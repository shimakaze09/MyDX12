...
MyDX12::MeshGPUBuffer::MeshGPUBuffer(
  ...
  DirectX::CreateStaticBuffer(device, resourceUpload, ib_data, ib_count,
                              ib_stride, D3D12_RESOURCE_STATE_GENERIC_READ,
...
MyDX12::MeshGPUBuffer::MeshGPUBuffer(ID3D12Device* device,
  ...
  Update(device, cmdList, vb_data, vb_count, vb_stride, ib_data, ib_count,
         ib_format);
}

void MyDX12::MeshGPUBuffer::Update(ID3D12Device* device,
                                   ID3D12GraphicsCommandList* cmdList,
                                   const void* vb_data, UINT vb_count,
                                   UINT vb_stride, const void* ib_data,
                                   UINT ib_count, DXGI_FORMAT ib_format) {
  ...
  if (staticVertexBuffer && staticVertexBuffer->GetDesc().Width >= vb_size)
    vertexUploadBuffer->CopyAssign(0, 0, vb_size, cmdList,
                                   staticVertexBuffer.Get());
  else
    vertexUploadBuffer->CopyConstruct(0, 0, vb_size, device, cmdList,
                                      D3D12_RESOURCE_STATE_GENERIC_READ,
                                      &staticVertexBuffer);

  if (staticIndexBuffer && staticIndexBuffer->GetDesc().Width >= ib_size)
    indexUploadBuffer->CopyAssign(0, 0, ib_size, cmdList,
                                  staticIndexBuffer.Get());
  else
    indexUploadBuffer->CopyConstruct(0, 0, ib_size, device, cmdList,
                                     D3D12_RESOURCE_STATE_GENERIC_READ,
                                     &staticIndexBuffer);
}

void MyDX12::MeshGPUBuffer::ConvertToStatic() {
  assert(!IsStatic());
...
void MyDX12::MeshGPUBuffer::UpdateAndConvertToStatic(
    ResourceDeleteBatch& deleteBatch, ID3D12Device* device,
    ID3D12GraphicsCommandList* cmdList, const void* vb_data, UINT vb_count,
    UINT vb_stride, const void* ib_data, UINT ib_count, DXGI_FORMAT ib_format) {
  assert(!IsStatic());
  ...
  if (staticVertexBuffer &&
      staticVertexBuffer->GetDesc().Width == vertexBufferByteSize) {
    vertexUploadBuffer->MoveAssign(0, 0, vertexBufferByteSize, deleteBatch,
                                   cmdList, staticVertexBuffer.Get());
  } else {
    vertexUploadBuffer->MoveConstruct(
        0, 0, vertexBufferByteSize, deleteBatch, device, cmdList,
        D3D12_RESOURCE_STATE_GENERIC_READ, &staticVertexBuffer);
  }

  if (staticIndexBuffer &&
      staticIndexBuffer->GetDesc().Width == indexBufferByteSize) {
    indexUploadBuffer->MoveAssign(0, 0, indexBufferByteSize, deleteBatch,
                                  cmdList, staticIndexBuffer.Get());
  } else {
    indexUploadBuffer->MoveConstruct(
        0, 0, indexBufferByteSize, deleteBatch, device, cmdList,
        D3D12_RESOURCE_STATE_GENERIC_READ, &staticIndexBuffer);
  }
