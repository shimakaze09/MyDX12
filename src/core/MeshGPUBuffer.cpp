...
void MyDX12::MeshGPUBuffer::ConvertToStatic(ResourceDeleteBatch& deleteBatch) {
  assert(!IsStatic());
...
void MyDX12::MeshGPUBuffer::ConvertToDynamic(ID3D12Device* device) {
  assert(IsStatic());
  isStatic = false;
...
void MyDX12::MeshGPUBuffer::Delete(ResourceDeleteBatch& deleteBatch) {
  if (!IsStatic()) {
    vertexUploadBuffer->Delete(deleteBatch);
    indexUploadBuffer->Delete(deleteBatch);
    vertexUploadBuffer.reset();
