...
MyDX12::DescriptorHeapAllocation::DescriptorHeapAllocation(
    IDescriptorAllocator* pAllocator, ID3D12DescriptorHeap* pHeap,
    D3D12_CPU_DESCRIPTOR_HANDLE CpuHandle,
    D3D12_GPU_DESCRIPTOR_HANDLE GpuHandle, uint32_t NHandles,
  ...
  assert("DescriptorSize exceeds allowed limit" &&
         DescriptorSize < std::numeric_limits<uint16_t>::max());
  m_DescriptorSize = static_cast<uint16_t>(DescriptorSize);
}

MyDX12::DescriptorHeapAllocation::DescriptorHeapAllocation(
    DescriptorHeapAllocation&& Allocation) noexcept
...
MyDX12::DescriptorHeapAllocation& MyDX12::DescriptorHeapAllocation::operator=(
    DescriptorHeapAllocation&& Allocation) noexcept {
  m_FirstCpuHandle = std::move(Allocation.m_FirstCpuHandle);
  m_FirstGpuHandle = std::move(Allocation.m_FirstGpuHandle);
  m_NumHandles = std::move(Allocation.m_NumHandles);
...
void MyDX12::DescriptorHeapAllocation::Reset() noexcept {
  m_FirstCpuHandle.ptr = 0;
  m_FirstGpuHandle.ptr = 0;
