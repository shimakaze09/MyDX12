    ...
    for (const auto& [rsrcNodeIdx, state_desc] : rsrcs) {
      ...
      std::visit(
          [&](const auto& desc) {
            ...
            // CBV
            if constexpr (std::is_same_v<T, D3D12_CONSTANT_BUFFER_VIEW_DESC>) {
              if (handles.desc2info_cbv.find(desc) !=
                  handles.desc2info_cbv.end())
                return;
              auto idx = csuDHfree.back();
              csuDHfree.pop_back();
              csuDHused.insert(idx);
              handles.desc2info_cbv[desc] = {csuDH.GetCpuHandle(idx),
            ...
            // SRV
              ...
              if constexpr (std::is_same_v<T,
                                           D3D12_SHADER_RESOURCE_VIEW_DESC>) {
                if (handles.desc2info_srv.find(desc) !=
              ...
              if constexpr (std::is_same_v<T, D3D12_SHADER_RESOURCE_VIEW_DESC>)
                handles.desc2info_srv[desc] = {csuDH.GetCpuHandle(idx),
                                               csuDH.GetGpuHandle(idx), false};
              else
                handles.null_info_srv = {csuDH.GetCpuHandle(idx),
                                         csuDH.GetGpuHandle(idx), false};
            }
            // UAV
            else if constexpr (std::is_same_v<
              ...
              if constexpr (std::is_same_v<T,
                                           D3D12_UNORDERED_ACCESS_VIEW_DESC>) {
                if (handles.desc2info_uav.find(desc) !=
              ...
              if constexpr (std::is_same_v<T, D3D12_UNORDERED_ACCESS_VIEW_DESC>)
                handles.desc2info_uav[desc] = {csuDH.GetCpuHandle(idx),
                                               csuDH.GetGpuHandle(idx), false};
              else
                handles.null_info_uav = {csuDH.GetCpuHandle(idx),
                                         csuDH.GetGpuHandle(idx), false};
            }
            // RTV
            else if constexpr (std::is_same_v<T,
              ...
              if constexpr (std::is_same_v<T, D3D12_RENDER_TARGET_VIEW_DESC>) {
                if (handles.desc2info_rtv.find(desc) !=
            ...
            // DSV
              ...
              if constexpr (std::is_same_v<T, D3D12_DEPTH_STENCIL_VIEW_DESC>) {
                if (handles.desc2info_dsv.find(desc) !=
...
PassRsrcs RsrcMngr::RequestPassRsrcs(ID3D12Device* device,
  ...
  for (const auto& [rsrcNodeIdx, state_desc] : rsrcStates) {
    ...
    passRsrc[rsrcNodeIdx] = std::visit(
        [&, rsrcNodeIdx = rsrcNodeIdx](const auto& desc) -> RsrcImpl {
          using T = std::decay_t<decltype(desc)>;
