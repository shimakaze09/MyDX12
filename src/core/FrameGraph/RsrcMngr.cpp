        ...
        std::visit(
            [&](const auto& desc) {
              ...
              // RTV
                                     ...
                                     T, D3D12_RENDER_TARGET_VIEW_DESC>) {
              ...
              // DTV
                                     ...
                                     T, D3D12_DEPTH_STENCIL_VIEW_DESC>) {
...
RsrcMngr& RsrcMngr::RegisterPassRsrcState(size_t passNodeIdx,
                                          size_t rsrcNodeIdx, RsrcState state) {
  std::get<RsrcState>(passNodeIdx2rsrcMap[passNodeIdx][rsrcNodeIdx]) = state;
  return *this;
}

RsrcMngr& RsrcMngr::RegisterPassRsrcImplDesc(size_t passNodeIdx,
                                             size_t rsrcNodeIdx,
                                             RsrcImplDesc desc) {
...
RsrcMngr& RsrcMngr::RegisterPassRsrc(size_t passNodeIdx, size_t rsrcNodeIdx,
      ...
      for (const auto& desc : descs) {
        std::visit(
            [&](const auto& desc) {
              ...
              if constexpr (std::is_same_v<T,
                                           D3D12_CONSTANT_BUFFER_VIEW_DESC>) {
                if (typeinfo.desc2info_cbv.find(desc) !=
                    typeinfo.desc2info_cbv.end())
                  return;
                auto idx = csuDHfree.back();
                csuDHfree.pop_back();
                csuDHused.insert(idx);
                typeinfo.desc2info_cbv[desc] = {csuDH.GetCpuHandle(idx),
                                                csuDH.GetGpuHandle(idx), false};
              }
              // SRV
              else if constexpr (std::is_same_v<
                                     T, D3D12_SHADER_RESOURCE_VIEW_DESC> ||
                ...
                if constexpr (std::is_same_v<T,
                                             D3D12_SHADER_RESOURCE_VIEW_DESC>) {
                  if (typeinfo.desc2info_srv.find(desc) !=
                ...
                if constexpr (std::is_same_v<T,
                                             D3D12_SHADER_RESOURCE_VIEW_DESC>)
                  typeinfo.desc2info_srv[desc] = {
                      csuDH.GetCpuHandle(idx), csuDH.GetGpuHandle(idx), false};
                else
                  typeinfo.null_info_srv = {csuDH.GetCpuHandle(idx),
                                            csuDH.GetGpuHandle(idx), false};
              }
              // UAV
              else if constexpr (std::is_same_v<
                ...
                if constexpr (std::is_same_v<
                                  T, D3D12_UNORDERED_ACCESS_VIEW_DESC>) {
                  if (typeinfo.desc2info_uav.find(desc) !=
                ...
                if constexpr (std::is_same_v<T,
                                             D3D12_UNORDERED_ACCESS_VIEW_DESC>)
                  typeinfo.desc2info_uav[desc] = {
                      csuDH.GetCpuHandle(idx), csuDH.GetGpuHandle(idx), false};
                else
                  typeinfo.null_info_uav = {csuDH.GetCpuHandle(idx),
                                            csuDH.GetGpuHandle(idx), false};
              }
              // RTV
              else if constexpr (std::is_same_v<
                ...
                if constexpr (std::is_same_v<T,
                  ...
                  if (typeinfo.desc2info_rtv.find(desc) !=
                ...
                if constexpr (std::is_same_v<T, D3D12_RENDER_TARGET_VIEW_DESC>)
                  typeinfo.desc2info_rtv[desc] = {rtvDH.GetCpuHandle(idx),
                                                  false};
                else
              ...
              // DSV
                ...
                if constexpr (std::is_same_v<T,
                  ...
                  if (typeinfo.desc2info_dsv.find(desc) !=
                ...
                if constexpr (std::is_same_v<T, D3D12_DEPTH_STENCIL_VIEW_DESC>)
                  typeinfo.desc2info_dsv[desc] = {dsvDH.GetCpuHandle(idx),
                                                  false};
                else
            ...
            if constexpr (std::is_same_v<T, D3D12_CONSTANT_BUFFER_VIEW_DESC>) {
              ...
              if (!info.init) {
                assert(desc.BufferLocation ==
                       ...
                       IsImported(rsrcNodeIdx) &&
                           desc.BufferLocation ==
                               view.pRsrc->GetGPUVirtualAddress());
...
bool RsrcMngr::CheckComplete(const MyFG::FrameGraph& fg) {
