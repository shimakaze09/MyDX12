...
void VarSizeAllocMngr::ResetCurrAlignment() noexcept {
  curMinAlignment = 1;
  while (curMinAlignment * 2 <= capacity) curMinAlignment *= 2;
}
...
VarSizeAllocMngr::Allocation VarSizeAllocMngr::Allocate(size_t size,
  ...
  if (freeSize < size) return Allocation::Invalid();

  auto alignmentReserve =
      (alignment > curMinAlignment) ? alignment - curMinAlignment : 0;
  // Get the first block that is large enough to encompass size +
  // alignmentReserve bytes lower_bound() returns an iterator pointing to the
  // first element that is not less (i.e. >= ) than key
  auto SmallestBlockItIt = size2freeblock.lower_bound(size + alignmentReserve);
  if (SmallestBlockItIt == size2freeblock.end()) return Allocation::Invalid();
...
void VarSizeAllocMngr::Free(size_t offset, size_t size) {
  ...
  assert(nextBlockIt == offset2freeblock.end() ||
  ...
  if (prevBlockIt != offset2freeblock.end() &&
    ...
    if (nextBlockIt != offset2freeblock.end() &&
        offset + size == nextBlockIt->first) {
  ...
  } else if (nextBlockIt != offset2freeblock.end() &&
             offset + size == nextBlockIt->first) {
    //   PrevBlock.Offset                   Offset            NextBlock.Offset
    //     |                                  |                    |
    //     |<-----PrevBlock.Size----->| ~ ~ ~
