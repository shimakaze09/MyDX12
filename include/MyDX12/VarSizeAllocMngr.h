...
// 2.
// https://github.com/DiligentGraphics/DiligentCore/blob/09b9f1dc1ec69683c3143f309e6e8a0141361624/Graphics/GraphicsAccessories/interface/VarSizeAllocMngr.hpp
// 3.
// https://github.com/DiligentGraphics/DiligentCore/blob/09b9f1dc1ec69683c3143f309e6e8a0141361624/Graphics/GraphicsEngineD3D12/src/DescriptorHeap.cpp
...
// The first map keeps blocks sorted by their offsets. The second multimap keeps
// blocks sorted by their sizes. The elements of the two maps reference each
// other, which enables efficient block insertion, removal and merging.
//
//   8                 32                       64                           104
//   |<---16--->|       |<-----24------>|        |<---16--->| |<-----32----->|
//
...
class VarSizeAllocMngr {
 public:
  ...
  // Offset returned by Allocate() may not be aligned, but the size of the
  // allocation is sufficient to properly align it
  struct Allocation {
    ...
    constexpr bool IsValid() const noexcept {
      return unalignedOffset != Invalid().unalignedOffset;
    }
 ...
 private:
  ...
  using TFreeBlocksBySizeMap =
      std::multimap<size_t, TFreeBlocksByOffsetMap::iterator>;
