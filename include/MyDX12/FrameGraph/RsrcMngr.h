...
class RsrcMngr {
 public:
  ...
  RsrcMngr& RegisterPassRsrcs(size_t passNodeIdx, size_t rsrcNodeIdx,
                              RsrcState state, RsrcImplDesc desc) {
    passNodeIdx2rsrcMap[passNodeIdx].emplace(rsrcNodeIdx,
                                             std::tuple(state, desc));
    return *this;
  }
 ...
 private:
  ...
  std::unordered_map<
      size_t, std::unordered_map<size_t, std::tuple<RsrcState, RsrcImplDesc>>>
      passNodeIdx2rsrcMap;
  ...
  struct DHHandles {
    ...
    std::unordered_map<D3D12_CONSTANT_BUFFER_VIEW_DESC, CpuGpuInfo>
        desc2info_cbv;
    std::unordered_map<D3D12_SHADER_RESOURCE_VIEW_DESC, CpuGpuInfo>
        desc2info_srv;
