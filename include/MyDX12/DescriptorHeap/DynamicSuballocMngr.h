...
class DynamicSuballocMngr final : public IDescriptorAllocator {
 public:
  DynamicSuballocMngr(GPUDescriptorHeap* ParentGPUHeap,
  ...
  virtual void Free(
      DescriptorHeapAllocation&& allocation) noexcept override final {
  ...
  size_t GetSuballocationCount() const noexcept {
    return m_Suballocations.size();
  }
