...
//
// Allocation routine goes through the list of managers that have available
// descriptors and tries to process the request using every manager. If there
// are no available managers or no manager was able to handle the request, the
// function creates a new descriptor heap manager and lets it handle the request
//
// Render device contains four CPUDescriptorHeap object instances (one for each
// D3D12 heap type). The heaps are accessed when a texture or a buffer view is
// created.
//
class CPUDescriptorHeap final : public IDescriptorAllocator {
 public:
  // Initializes the heap
  CPUDescriptorHeap(ID3D12Device* pDevice, uint32_t NumDescriptorsInHeap,
