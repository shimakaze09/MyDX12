...
// The second part is intended to provide temporary storage for dynamic
// resources. Space for dynamic resources is allocated in chunks, and then
// descriptors are suballocated within every chunk. DynamicSuballocMngr
...
//                                     unused
//  | X O O X X O X O O O O X X X X O  ||  | X X X O | | X X O O | | O O O O |
//  O O O O  ||
//                                               |         |
//                                     suballocation       suballocation
//                                    within chunk 0       within chunk 1
//
// Render device contains two GPUDescriptorHeap instances (CBV_SRV_UAV and
// SAMPLER). The heaps are used to allocate GPU-visible descriptors for shader
// resource binding objects. The heaps are also used by the command contexts
// (through DynamicSuballocMngr to allocated dynamic descriptors)
//
//  _______________________________________________________________________________________________________________________________
// | Render Device | | | | m_CPUDescriptorHeaps[CBV_SRV_UAV] |  X  X  X  X  X  X
// X  X  |, |  X  X  X  X  X  X  X  X  |, |  X  O  O  X  O  O  O  O  |    | |
// m_CPUDescriptorHeaps[SAMPLER]     |  X  X  X  X  O  O  O  X  |, |  X  O  O  X
// O  O  O  O  |                                  | | m_CPUDescriptorHeaps[RTV]
// |  X  X  X  O  O  O  O  O  |, |  O  O  O  O  O  O  O  O  | | |
// m_CPUDescriptorHeaps[DSV]         |  X  X  X  O  X  O  X  O  | | | ctx1 ctx2
// | | m_GPUDescriptorHeaps[CBV_SRV_UAV]  | X O O X X O X O O O O X X X X O  ||
// | X X X O | | X X O O | | O O O O |  O O O O  ||    | |
// m_GPUDescriptorHeaps[SAMPLER]      | X X O O X O X X X O O X O O O O  ||  | X
// X O O | | X O O O | | O O O O |  O O O O  ||    | | |
// |_______________________________________________________________________________________________________________________________|
//
//  ________________________________________________
//  ________________________________________________
// |Device Context 1                                |             |Device
// Context 2                                | | |             | | |
...
class GPUDescriptorHeap final : public IDescriptorAllocator {
 public:
  GPUDescriptorHeap(ID3D12Device* pDevice, uint32_t NumDescriptorsInHeap,
